<form action="/posts" method="GET" class="max-w-lg  mb-5">
    @if (request('category'))
    ...
    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
        ...
        <div id="dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
                ...
                <li>
                    <button onclick="window.location.href='/posts?category={{ $item->slug }}'" type="button" class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ $item->name }}</button>
                </li>
                @endforeach
            </ul>
        </div>
